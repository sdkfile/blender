...
static const EnumPropertyItem target_space_pchan_items[] = {
    ...
    {CONSTRAINT_SPACE_OWNLOCAL,
     ...
     "system, compensating for the difference in rest pose orientations to produce the same "
     "global motion when applied to the owner bone if the parents are still in rest pose"},
    {0, NULL, 0, NULL, NULL},
};
...
static void rna_def_constraint_transform_like(BlenderRNA *brna)
{
  ...
  RNA_def_property_ui_text(
      prop, "Fix Target Shear", "Remove shear from the target transformation before combining");
  RNA_def_property_update(prop, NC_OBJECT | ND_CONSTRAINT, "rna_Constraint_update");
